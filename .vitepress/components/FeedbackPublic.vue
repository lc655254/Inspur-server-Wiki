  ...
  methods: {
    ...
    async loadList() {
      ...
      try {
        ...
        const res = await fetch('https://api.inspurs.work/api/feedback/public/list', { headers });
    ...
    async openDetail(fb) {
      ...
      if (token) headers['Authorization'] = 'Bearer ' + token;
      try {
        const res = await fetch(`https://api.inspurs.work/api/feedback/public/${fb.id}`, { headers });
        const data = await res.json();
        if (data.code === 200) {
    ...
    async voteFeedback(dir) {
      ...
      const token = localStorage.getItem('token');
      try {
        const res = await fetch(`https://api.inspurs.work/api/feedback/${this.selectedFeedback.id}/vote`, {
          method: 'POST',
        ...
        if (data.code === 200) {
          ...
          const vRes = await fetch(`https://api.inspurs.work/api/feedback/${this.selectedFeedback.id}/votes`, {
    ...
    async voteComment(comment, dir) {
      ...
      const token = localStorage.getItem('token');
      try {
        const res = await fetch(`https://api.inspurs.work/api/comment/${comment.id}/vote`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + token },
          body: JSON.stringify({ vote: dir })
        });
        ...
        if (data.code === 200) {
          ...
          const vRes = await fetch(`https://api.inspurs.work/api/comment/${comment.id}/votes`, {
            headers: { 'Authorization': 'Bearer ' + token }
          });
          const vData = await vRes.json();
    ...
    async submitComment() {
      ...
      const token = localStorage.getItem('token');
      try {
        const res = await fetch(`https://api.inspurs.work/api/feedback/${this.selectedFeedback.id}/comment`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + token },
    ...
    async deleteComment(commentId) {
      ...
      const token = localStorage.getItem('token');
      try {
        await fetch(`https://api.inspurs.work/api/comment/${commentId}`, {
    ...
    async toggleHide(fb) {
      ...
      try {
        await fetch(`https://api.inspurs.work/api/feedback/${fb.id}/hide`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + token },
